...
Protonek::Protonek(const char* port, int baud) : rspeed(0), lspeed(0), isBalancing(false)
{
...
void Protonek::AddPortName(const char* port)
{
        portNames.push_back(port);
}

bool Protonek::openPort()
{
        for (std::vector<const char*>::iterator i = portNames.begin(); i != portNames.end(); ++i)
        {
                fd = open(*i, O_RDWR | O_NOCTTY | O_NDELAY);
                if (fd==-1) {
...
void Protonek::closePort() {
  // restore old port settings
  if (fd > 0) tcsetattr(fd, TCSANOW, &oldtio);
  close(fd);
}

bool Protonek::openJoystick() {
    // open the joystick
    if( ( joy.joy_fd = open( JOY_DEV, O_RDONLY ) ) < 0 )
    {
    ...
    ioctl( joy.joy_fd, JSIOCGNAME(80), &joy.name_of_joystick );
    ...
    fcntl( joy.joy_fd, F_SETFL, O_NONBLOCK );	// use non-blocking mode

    return true;
}

void Protonek::closeJoystick() {
    close(joy.joy_fd);
}
                ...
                //if (ans_i == sizeof(MSG))
                {
                    uint16_t crc = Crc::Crc16(answer);
                        ...
                        if (crc==0 && answer.header==MSG_HEADER && answer.adr==1)	// crc
                        {
                                ...
                                if (0)
                                        printf("%d %dms %d %d %d %.3f / %.3f %d moc: %.3f / %.3f dist: %d\n",
                                                (int)answer.adr,
